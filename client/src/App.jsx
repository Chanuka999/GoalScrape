...
const App = () => {
      ...
      <div className="wc-widget">
        ...
        <div className="wc-countdown">
          <div className="time-block">
            <span className="val">{wcTimeLeft.d}</span>{" "}
            <span className="lbl">DAYS</span>
          </div>
          <div className="time-block">
            <span className="val">{wcTimeLeft.h}</span>{" "}
            <span className="lbl">HRS</span>
          </div>
          <div className="time-block">
            <span className="val">{wcTimeLeft.m}</span>{" "}
            <span className="lbl">MINS</span>
          </div>
          <div className="time-block">
            <span className="val">{wcTimeLeft.s}</span>{" "}
            <span className="lbl">SECS</span>
          </div>
        </div>
        <div className="wc-footer">
          <div className="pulse-green inline-pulse"></div> Phase: Global
          Regional Qualifiers Active
        </div>
      </div>
    );
  };
      ...
      <div className="history-page">
        ...
        <div className="history-grid">
          {loading ? (
            ...
            <div className="no-data-msg">No history data found.</div>
          ) : (
            historyData.map((wc) => (
              <div key={wc.year} className="history-card">
                ...
                <div className="final-match-box">
                  <div className="h-team winner">
                    <span className="flag">{wc.img}</span> {wc.winner}{" "}
                    <span className="crown" title="Champion">
                      👑
                    </span>
                  </div>
                  ...
                  <div className="h-team runner">{wc.runnerUp}</div>
                </div>
                <div className="wc-details-list">
                  {wc.goldenBoot && (
                    <div className="wc-detail-row">
                      <b>Golden Boot:</b> {wc.goldenBoot}
                    </div>
                  )}
                  {wc.goldenBall && (
                    <div className="wc-detail-row">
                      <b>Golden Ball:</b> {wc.goldenBall}
                    </div>
                  )}
                  {wc.totalGoals && (
                    <div className="wc-detail-row">
                      <b>Total Goals:</b> {wc.totalGoals}
                    </div>
                  )}
                  {wc.attendance && (
                    <div className="wc-detail-row">
                      <b>Attendance:</b> {wc.attendance}
                    </div>
                  )}
                  {wc.stadium && (
                    <div className="wc-detail-row">
                      <b>Stadium:</b> {wc.stadium}
                    </div>
                  )}
                </div>
              </div>
  ...
  const getAIAnalysis = (match) => {
    try {
      ...
      if (timeStr.includes("finished")) {
        return h > a
          ? { h: 100, a: 0, d: 0, v: match["Home Team"] }
          : a > h
            ? { h: 0, a: 100, d: 0, v: match["Away Team"] }
            : { h: 0, a: 0, d: 100, v: "Draw" };
      }
  ...
  const favoriteMatches = matches.filter((match) =>
    favorites.includes(
      `${match.League}-${match["Home Team"]}-${match["Away Team"]}`,
    ),
  );
  ...
  const AIAnalysisModal = ({ match, onClose }) => {
      ...
      datasets: [
        {
          data: [
            ...
            analysis.away || analysis.a,
            analysis.draw || analysis.d,
          ],
    ...
    const pseudoRandom = (seedInfo, index, max) => {
      let hash = 0;
      for (let i = 0; i < seedInfo.length; i++)
        hash = seedInfo.charCodeAt(i) + ((hash << 5) - hash);
    ...
    const generateMatchTimeline = (match) => {
      ...
      for (let i = 0; i < h; i++)
        events.push({
          minute: pseudoRandom(seed, i * 5, 90),
          team: "home",
          type: "goal",
        });
      for (let i = 0; i < a; i++)
        events.push({
          minute: pseudoRandom(seed, i * 7 + 11, 90),
          team: "away",
          type: "goal",
        });
      ...
      for (let i = 0; i < cards; i++) {
        ...
        events.push({
          minute: pseudoRandom(seed, 30 + i, 90),
          team: t,
          type: "yellow",
        });
      }
      ...
      return (
        <div className={`team-form-block ${alignment}`}>
          ...
          <div className="form-badges">
            {form.map((res, i) => (
              <span key={i} className={`f-badge ${res}`}>
                {res}
              </span>
            ))}
          </div>
    ...
    const TimelineView = () => (
      <div
        className="match-stats-visualizer mt-4"
      ...
      >
        ...
        <div className="timeline-container">
          ...
          {timelineEvents.length === 0 && (
            <div className="no-events text-center mt-2 text-gray-500 text-sm">
    ...
    return (
      <div className="modal-overlay" onClick={onClose}>
        <div
          className="modal-content analytics"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="modal-header">
            <h3>
              {isFinished ? <Trophy size={20} /> : <BarChart3 size={20} />}{" "}
              {isFinished ? "Match Summary" : "AI Match Analysis"}
            </h3>
            ...
            {!isFinished ? (
              <>
                ...
                <div className="prob-bars-stack">
                  <div className="prob-bar-label">
                    ...
                    <span>{analysis.home || analysis.h}%</span>
                  </div>
                  <div className="progress-bg">
                    <div
                      className="progress-fill home"
                      style={{ width: `${analysis.home || analysis.h}%` }}
                    ></div>
                  </div>
                  <div className="prob-bar-label">
                    <span>Draw</span>
                    <span>{analysis.draw || analysis.d}%</span>
                  </div>
                  <div className="progress-bg">
                    <div
                      className="progress-fill draw"
                      style={{ width: `${analysis.draw || analysis.d}%` }}
                    ></div>
                  </div>
                  <div className="prob-bar-label">
                    <span>{match["Away Team"]}</span>
                    <span>{analysis.away || analysis.a}%</span>
                  </div>
                  <div className="progress-bg">
                    <div
                      className="progress-fill away"
                      style={{ width: `${analysis.away || analysis.a}%` }}
                    ></div>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="stats-list">
                    <StatRow
                      label="Possession"
                      home={simulatedStats.possession[0]}
                      away={simulatedStats.possession[1]}
                      isPercent={true}
                    />
                    <StatRow
                      label="Shots"
                      home={simulatedStats.shots[0]}
                      away={simulatedStats.shots[1]}
                    />
                    <StatRow
                      label="Corners"
                      home={simulatedStats.corners[0]}
                      away={simulatedStats.corners[1]}
                    />
                    <StatRow
                      label="Fouls"
                      home={simulatedStats.fouls[0]}
                      away={simulatedStats.fouls[1]}
                    />
                    <StatRow
                      label="Yellow cards"
                      home={simulatedStats.cards[0]}
                      away={simulatedStats.cards[1]}
                    />
                  </div>
                </div>
              </>
              ...
              <div className="finished-match-summary">
                <div className="result-hero">
                  ...
                  <div className="hero-team text-right">
                    <span>{match["Away Team"]}</span>
                  </div>
                </div>
          ...
          <div className="modal-footer-info">
            <Clock size={14} />{" "}
            {isFinished
              ? "Match database archived"
              : "Next outcome update in 60s"}
          </div>
        </div>
      </div>
    );
  ...
  const MatchCard = ({ match }) => {
    ...
    return (
      <div
        className={`match-card ai-powered ${isFav ? "premium-fav" : ""} ${isFinished ? "finished" : ""}`}
  ...
  const finalFilteredMatches = useMemo(() => {
    ...
    if (activeTab === "Live")
      return base.filter(
        (m) =>
          !m.Time.includes(":") && !m.Time.toLowerCase().includes("finished"),
      );
    if (activeTab === "Finished")
      return base.filter((m) => m.Time.toLowerCase().includes("finished"));
    if (MAJOR_LEAGUES.includes(activeTab))
      return base.filter((m) =>
        m.League.toLowerCase().includes(activeTab.toLowerCase()),
      );
    ...
    <div className="dashboard dark-ai">
      <header className="main-header">
        ...
        <div className="header-actions">
          ...
          <button
            className={`action-btn analytics ${showAnalytics ? "active" : ""}`}
          ...
          <button
            className={`action-btn analytics ${appView === "history" ? "active" : ""}`}
            onClick={() =>
              setAppView(appView === "history" ? "dashboard" : "history")
            }
          >
          ...
          <button
            className={`action-btn scrape ${scraping ? "loading" : ""}`}
          ...
          <div className="tabs-bar">
            ...
            <div className="league-dropdown-container">
              <select
                ...
                value={
                  MAJOR_LEAGUES.includes(activeTab) ? activeTab : "default"
                }
              >
            ...
            {activeTab === "Standings" ? (
              <div className="standings-wrapper">
                {Object.keys(standingsData).map((league) => (
                  <section key={league} className="category-section">
                    <h2 className="section-title">
                      <Trophy size={20} color="#00ff80" /> {league} - Standings
                    </h2>
                          ...
                          {standingsData[league].map((t, i) => (
                            <tr
                              ...
                              className={
                                i < 4
                                  ? "promo-zone"
                                  : i >= standingsData[league].length - 3 &&
                                      standingsData[league].length > 4
                                    ? "rel-zone"
                                    : ""
                              }
                            >
                ...
                {Object.keys(standingsData).length === 0 && (
                  <div className="no-data-msg text-center mt-8 text-gray-500">
                    No league standings available in the database currently.
                  </div>
                )}
              </div>
            ) : (
              <>
                {favMatches.length > 0 && activeTab === "All" && (
                  <section className="category-section favorites-area">
                    <h2 className="section-title">
                      <Star className="fill-yellow" size={24} /> Pinned
                      Analytics
                    </h2>
                    <div className="matches-grid">
                      {favMatches.map((match, idx) => (
                        <MatchCard key={`fav-${idx}`} match={match} />
                      ))}
                    </div>
                  </section>
                )}
                ...
                {Object.keys(finalGroups).map((league) => (
                  <section key={league} className="category-section">
                    <h2 className="section-title">
                      <ChevronRight size={20} /> {league}
                    </h2>
                    <div className="matches-grid">
                      {finalGroups[league].map((match, idx) => (
